...
const MaysanDohaLXR = () => {
  ...
  const getAnimationClass = (section: string) => {
    return isVisible[section] ? `animate-fade-in opacity-100 transform translate-y-0` : `opacity-0 transform translate-y-8`;
  };
  ...
  return <div className="bg-background">
      ...
      <section className="relative h-[32rem] bg-black overflow-hidden">
        ...
        backgroundImage: `url(${maysanDohaImage})`
      }}>
        ...
        <div className="relative z-10 h-full flex items-center justify-center text-center">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            animationDelay: '300ms'
              ...
              <p className="text-lg font-gotham text-white/95 max-w-2xl mx-auto leading-relaxed mb-12 transform transition-all duration-500" style={{
              animationDelay: '600ms'
            }}>
                A private sanctuary of wellness and heritage where Arabian craft meets contemporary calm.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center transform transition-all duration-500" style={{
      ...
      <section className="py-24 bg-gradient-to-b from-paper via-sand/20 to-paper" data-section="overview">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-16 items-start">
            ...
            <div className={`lg:col-span-3 transition-all duration-700 ${getAnimationClass('overview')}`} style={getStaggerDelay(0)}>
              <div className="space-y-8">
                <div className="maysan-eyebrow font-gotham-medium text-katara-muted mb-4 tracking-wide" style={getStaggerDelay(0)}>
                  Overview
                </div>
                <h2 className="maysan-h2 font-gotham-bold text-ink mb-6 transform transition-all duration-500 hover:text-brand-orange" style={getStaggerDelay(1)}>
      ...
      <section className="py-20 bg-gradient-to-r from-sand via-sand/90 to-sand" data-section="rituals">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            number: 1,
            title: "Arrive",
            description: "Enter a world of calm, where the city's energy gives way to tranquil spaces designed for reflection."
          }, {
            number: 2,
            title: "Restore",
            description: "Renew mind and body through holistic treatments and thermal experiences crafted for deep wellness."
          }, {
            number: 3,
            title: "Reconnect",
            description: "Return refreshed, carrying a renewed sense of balance that extends beyond your visit."
          }].map((step, index) => <div key={index} className={`text-center group transition-all duration-700 transform hover:scale-105 hover:-translate-y-2 ${isVisible.rituals ? 'animate-fade-in' : 'opacity-0'} ${ritualStep === index ? 'scale-105 shadow-lg' : ''}`} style={getStaggerDelay(index)} onMouseEnter={() => setRitualStep(index)} onMouseLeave={() => setRitualStep(-1)}>
                <div className="relative mb-8">
                  <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 transition-all duration-500 ${ritualStep === index ? 'bg-gradient-to-br from-brand-orange to-brand-orange/70 scale-110' : 'bg-gradient-to-br from-brand-orange/20 to-brand-orange/10 group-hover:from-brand-orange/30 group-hover:to-brand-orange/20'}`}>
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${ritualStep === index ? 'bg-white' : 'bg-brand-orange/30 group-hover:bg-brand-orange/40'}`}>
                      <span className={`font-gotham-bold text-lg transition-all duration-300 ${ritualStep === index ? 'text-brand-orange' : 'text-brand-orange'}`}>{step.number}</span>
                    </div>
                  </div>
                  {index < 2 && <div className="absolute top-20 left-1/2 transform -translate-x-1/2 w-px h-12 bg-gradient-to-b from-brand-orange/30 to-transparent hidden md:block transition-all duration-500 group-hover:from-brand-orange/60"></div>}
                </div>
                <h3 className={`maysan-h3 font-gotham-bold mb-3 transition-all duration-300 ${ritualStep === index ? 'text-brand-orange' : 'text-ink group-hover:text-brand-orange'}`}>{step.title}</h3>
      ...
      <section className="py-24 bg-gradient-to-b from-paper to-sand/10" data-section="experiences">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-20 transition-all duration-700 ${getAnimationClass('experiences')}`}>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            icon: ChefHat
          }].map((experience, index) => <div key={index} className={`group transform transition-all duration-700 bg-white border border-line/50 rounded-sm overflow-hidden ${isVisible.experiences ? 'animate-fade-in' : 'opacity-0'} ${hoveredCard === index ? 'scale-105 shadow-2xl -translate-y-4' : 'hover:shadow-lg hover:-translate-y-2'}`} style={getStaggerDelay(index)} onMouseEnter={() => setHoveredCard(index)} onMouseLeave={() => setHoveredCard(null)}>
                <div className={`aspect-[4/3] bg-gradient-to-br from-sand to-sand/60 border-b border-line/30 relative overflow-hidden transition-all duration-500 ${hoveredCard === index ? 'scale-110' : ''}`}>
                  {index === 0 ? <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{
                backgroundImage: `url(/lovable-uploads/904ac2a9-edd7-43c2-85d6-694d225075aa.png)`
              }}>
                      <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-black/10 to-transparent"></div>
                    </div> : index === 1 ? <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{
                backgroundImage: `url(/lovable-uploads/f2b59a6f-76b2-4025-b755-29b0066f829d.png)`
              }}>
                      <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-black/10 to-transparent"></div>
                    </div> : index === 2 ? <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{
                backgroundImage: `url(/lovable-uploads/45618c6e-d114-407d-8136-efdc0c5dc75c.png)`
              }}>
                      <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-black/10 to-transparent"></div>
                    </div> : <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent"></div>}
                  <div className={`absolute bottom-4 left-4 transition-all duration-500 ${hoveredCard === index ? 'scale-125' : ''}`}>
                    <div className="w-8 h-8 bg-brand-orange/20 rounded-full flex items-center justify-center">
                      <experience.icon className={`w-4 h-4 transition-all duration-300 ${hoveredCard === index ? 'text-brand-orange' : 'text-brand-orange/60'}`} />
                    </div>
                  </div>
                </div>
                <div className="p-8">
                  <h3 className={`maysan-h3 font-gotham-bold mb-4 transition-all duration-300 ${hoveredCard === index ? 'text-brand-orange' : 'text-ink group-hover:text-brand-orange'}`}>
                    {experience.title}
                  </h3>
                  <p className="maysan-small font-gotham text-katara-muted leading-relaxed transition-all duration-300 group-hover:text-ink">
      ...
      <section className="py-20 bg-sand/30" data-section="location">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className={`bg-white border border-line/50 p-8 rounded-sm shadow-sm transform transition-all duration-700 hover:shadow-lg hover:scale-105 ${getAnimationClass('location')}`} style={getStaggerDelay(1)}>
              <div className="space-y-6">
                {[{
                location: "Doha waterfront",
                time: "10 min",
                icon: Waves
              }, {
                location: "Cultural venues",
                time: "8–12 min",
                icon: Landmark
              }, {
                location: "Airport",
                time: "25 min",
                icon: Plane
              }].map((item, index) => <div key={index} className={`flex items-center justify-between p-4 bg-sand/40 rounded-sm transform transition-all duration-500 hover:bg-sand/60 hover:scale-105 ${isVisible.location ? 'animate-fade-in' : 'opacity-0'}`} style={{
      ...
      <section className="relative py-32 bg-black overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="animate-fade-in transform transition-all duration-700" style={{
            animationDelay: '400ms'
              ...
              <p className="text-lg font-gotham text-white/90 mb-12 max-w-2xl mx-auto leading-relaxed" style={{
              animationDelay: '600ms'
            }}>
